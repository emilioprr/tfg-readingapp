            ...
            <div className="flex items-center justify-between mb-6">
                <div>
                    ...
                    <div className="flex gap-3 mt-2 text-xs">
                        {lista.esPublica ? <span className="bg-emerald-500/20 text-emerald-400 px-2 py-1 rounded-full">Pública</span>
                            : <span className="bg-dark-elevated text-dark-muted px-2 py-1 rounded-full">Privada</span>}
                        {lista.esAutomatica && <span className="bg-terra/20 text-terra px-2 py-1 rounded-full">Automática</span>}
                        <span className="text-dark-muted">{libros.length} libros</span>
                    </div>
                </div>
                {esMia && !lista.esAutomatica && (
                    <button onClick={eliminarLista} className="text-red-400 hover:text-red-300 text-sm transition-colors">Eliminar lista</button>
                )}
            </div>
            {libros.length === 0 ? <p className="text-dark-muted">Esta lista está vacía</p> : (
                        ...
                        <div key={libro.idlibro} className="group relative">
                            ...
                            {esMia && !lista.esAutomatica && (
                                <button onClick={() => quitarLibro(libro.idlibro)}
                                        className="absolute top-1 right-1 bg-black/60 text-red-400 hover:text-red-300 rounded-full w-6 h-6 flex items-center justify-center text-xs opacity-0 group-hover:opacity-100 transition-opacity">✕</button>
                            )}
                        </div>
                    ))}
                </div>
